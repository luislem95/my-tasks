...
import CollEstimated from './CollEstimated';

const EditEstimated = ({type}) => {
    const classes= useStyle();
    const[open,setOpen]=useState(false);
  return (
    <div className={classes.root}>
        <Collapse className={classes.buttonOpen} in={open} setOpen={setOpen}>
        <button className={classes.cross} onClick={()=>setOpen(false)}>x</button>
        <CollEstimated/>
        
        </Collapse>
        <Collapse className={classes.button} in={!open}> 
         <button className={classes.center} type={type} onClick={()=>setOpen(true)}>Estimated</button>
         </Collapse>
    </div>
  )
}
